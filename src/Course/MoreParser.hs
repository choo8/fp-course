...
between ::
  ...
  -> Parser a
  -> Parser a
between o c a = o >>= (\_ -> a >>= (\a' -> c >>= (\_ -> pure a')))
...
hex ::
  Parser Char
hex = replicateA 4 (satisfy isHexDigit) >>= (\h -> case readHex h of 
                                                        Empty  -> valueParser (chr 0)
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 p s = p >>= (\c -> list (s *> p) >>= (\cs -> pure (c:.cs)))

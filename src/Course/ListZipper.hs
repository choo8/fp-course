...
instance Functor ListZipper where
  (<$>) f (ListZipper l1 a l2) = ListZipper (f <$> l1) (f a) (f <$> l2)
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l1 a l2) = reverse l1 ++ (a :. Nil) ++ l2 
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper l1 a l2) = ListZipper l1 (f a) l2 
...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus a (ListZipper l1 _ l2) = ListZipper l1 a l2 
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper l1 a l2) = case break f l1 of
                                       (_, Nil)        -> IsNotZ
                                       (l2', (x:.l1')) -> IsZ (ListZipper l1' x (reverse l2' ++ (a:.l2))) 
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper l1 a l2) = case break f l2 of
                                        (_, Nil)        -> IsNotZ
                                        (l1', (x:.l2')) -> IsZ (ListZipper (reverse l1' ++ (a:.l1)) x l2')
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil a l2)     = ListZipper l1' x Nil
  where comb     = reverse (a:.l2)
        (x:.l1') = comb
moveLeftLoop (ListZipper (x:.l1) a l2) = ListZipper l1 x (a:.l2)  
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l1 a Nil)     = ListZipper Nil x l2'
  where comb = (reverse l1) ++ (a:.Nil)
        (x:.l2') = comb
moveRightLoop (ListZipper l1 a (x:.l2)) = ListZipper (a:.l1) x l2  
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft lz = if hasLeft lz then IsZ (moveLeftLoop lz) else IsNotZ
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight lz = if hasRight lz then IsZ (moveRightLoop lz) else IsNotZ
...
swapLeft (ListZipper (x:.l1) a l2) = IsZ (ListZipper (a:.l1) x l2)
...
swapRight (ListZipper l1 a (x:.l2)) = IsZ (ListZipper l1 x (a:.l2))
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ a l2) = ListZipper Nil a l2
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper l1 a _) = ListZipper l1 a Nil 
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n (ListZipper l1 a l2)
  | n == 0    = IsZ (ListZipper l1 a l2)
  | n < 0     = moveRightN (-n) (ListZipper l1 a l2)
  | otherwise = case moveLeft (ListZipper l1 a l2) of
                     IsNotZ -> IsNotZ
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n (ListZipper l1 a l2)
  | n == 0    = IsZ (ListZipper l1 a l2)
  | n < 0     = moveLeftN (-n) (ListZipper l1 a l2)
  | otherwise = case moveRight (ListZipper l1 a l2) of
                     IsNotZ -> IsNotZ
...
moveLeftN' n (ListZipper l1 a l2) = case moveLeftN n (ListZipper l1 a l2) of
                                         IsNotZ -> if n < 0 then Left (length l2) else Left (length l1)
                                         IsZ lz -> Right lz  
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n (ListZipper l1 a l2) = case moveRightN n (ListZipper l1 a l2) of
                                     IsNotZ -> if n < 0 then Left (length l1) else Left (length l2)
                                     IsZ lz -> Right lz 
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n (ListZipper l1 a l2) = moveRightN (n - (length l1)) (ListZipper l1 a l2)
...
index ::
  ListZipper a
  -> Int
index (ListZipper l1 _ _) = length l1
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l1 a l2) = lz
  where (IsZ lz) = nth ((length l1) + (length l2)) (ListZipper l1 a l2) 
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l1 a l2) = lz
  where (IsZ lz) = nth 0 (ListZipper l1 a l2) 
...
deletePullLeft (ListZipper (x:.l1) _ l2) = IsZ (ListZipper l1 x l2) 
...
deletePullRight (ListZipper l1 _ (x:.l2)) = IsZ (ListZipper l1 x l2) 
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper l1 a l2) = ListZipper (a:.l1) x l2 
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper l1 a l2) = ListZipper l1 x (a:.l2) 
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fl1 f fl2) (ListZipper l1 a l2) = ListZipper (zipWith ($) fl1 l1) (f a) (zipWith ($) fl2 l2)
...
instance Extend ListZipper where
  (<<=) f lz = ListZipper (unfoldr left lz) (f lz) (unfoldr right lz)
    where left x = case moveLeft x of
                        IsNotZ -> Empty
                        IsZ lz' -> Full (f lz', lz')
          right x = case moveRight x of
                         IsNotZ -> Empty
                         IsZ lz' -> Full (f lz', lz')
...
instance Traversable ListZipper where
  traverse f (ListZipper l1 a l2) = (ListZipper .reverse) <$> traverse f (reverse l1) <*> f a <*> traverse f l2
